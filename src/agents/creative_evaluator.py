"""
src/agents/creative_evaluator.py
Creative Evaluator Agent (CHS).

Responsibilities:
- Compute Creative Health Score (CHS) per campaign using:
  * behavior_score (ROAS/CTR performance vs other campaigns)
  * text_quality_score (benefit/urgency/social-proof language from text_terms)
  * fatigue_score (creative repetition: impression_share_of_top_creative)
- Provide CHS summaries (prev vs recent window).
- Enrich creative-related hypotheses with CHS evidence:
  * chs_prev, chs_recent, chs_delta, component scores
  * creative_confidence derived from CHS drop.
"""
...
class CreativeEvaluatorAgent:
    ...
    def run_creative_evaluation(
        ...
        for hyp in hypotheses:
            ...
            if chs_delta is None:
                ...
                creative_confidence = 0.4 + 0.4 * drop_factor  # up to ~0.8
            else:
                # CHS improved or stable; creative not likely the main issue
                creative_confidence = 0.2
    ...
    def _build_chs_summary(self, data_summary: Dict[str, Any]) -> Dict[str, Dict[str, Any]]:
        ...
        for cname, series in daily_by_campaign.items():
            ...
            if prev_impr + recent_impr < self.config["min_impressions_for_stats"]:
                # Insufficient volume to trust behavior component
                continue
        ...
        fatigue_by_campaign = {r["campaign_name"]: r for r in creative_repetition}

        for cname, stats in campaign_stats.items():
            behavior_prev = behavior_scores[cname]["behavior_prev"]
            behavior_recent = behavior_scores[cname]["behavior_recent"]
            ...
            # 5) Combine into CHS
            bw = self.config["behavior_weight"]
            tw = self.config["text_weight"]
            fw = self.config["fatigue_weight"]
            # normalize weights
            total_w = bw + tw + fw
            if total_w <= 0:
                bw = 0.5
    ...
    def _compute_behavior_scores(self, campaign_stats: Dict[str, Dict[str, Any]]) -> Dict[str, Dict[str, float]]:
        ...
        """
        prev_roas_vals = [s["prev_roas"] for s in campaign_stats.values() if s["prev_roas"] is not None]
        recent_roas_vals = [s["recent_roas"] for s in campaign_stats.values() if s["recent_roas"] is not None]
        prev_ctr_vals = [s["prev_ctr"] for s in campaign_stats.values() if s["prev_ctr"] is not None]
        recent_ctr_vals = [s["recent_ctr"] for s in campaign_stats.values() if s["recent_ctr"] is not None]
        ...
        for cname, s in campaign_stats.items():
            roas_prev_pct = percentile(s["prev_roas"], prev_roas_vals)
            roas_recent_pct = percentile(s["recent_roas"], recent_roas_vals)
            ctr_prev_pct = percentile(s["prev_ctr"], prev_ctr_vals)
            ctr_recent_pct = percentile(s["recent_ctr"], recent_ctr_vals)
    ...
    def _compute_text_quality(self, campaign_name: str, terms: List[Dict[str, Any]]) -> float:
        ...
        total_count = sum(t["count"] for t in terms)
        if total_count <= 0:
            return 0.5

        benefit_count = sum(t["count"] for t in terms if t["term"] in benefit_words)
        urgency_count = sum(t["count"] for t in terms if t["term"] in urgency_words)
        social_count = sum(t["count"] for t in terms if t["term"] in social_words)
